...
export function SaveFavoritePlaceButton({ anchor }: { anchor: TripAnchor }) {
  ...
  const favoritesKey = [...savedPlacesQueryKey, session?.user?.id];
  const [open, setOpen] = useState(false);
  const [label, setLabel] = useState(anchor.name.slice(0, 80));
  const [kind, setKind] = useState<SavedPlaceKind>("CUSTOM");
  const nameId = useId();
  const queryClient = useQueryClient();
  const favorites = useQuery({ queryKey: favoritesKey, queryFn: ({ signal }) => listSavedPlaces(signal), enabled: isAuthenticated, staleTime: 60_000, retry: 1 });
  const save = useMutation({
    mutationFn: createSavedPlace,
    onSuccess: (place) => {
      queryClient.setQueryData<SavedPlace[]>(favoritesKey, (previous) => [
  ...
  const isSaved = isAuthenticated && (anchor.kind === "SAVED_PLACE" || save.isSuccess || favorites.data?.some((place) =>
  ...
  return <>
    <Button type="button" variant="outline" size="sm" disabled={isAuthenticationLoading || Boolean(isSaved)} onClick={() => requireAuth(() => setOpen(true))}>
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!save.isPending) setOpen(next); }}>
      <DialogContent className="sm:max-w-md">
        ...
        <form className="space-y-4" onSubmit={(event) => {
          ...
          requireAuth(() => save.mutate({ label: label.trim(), kind, name: anchor.name.slice(0, 255),
          ...
          <Button type="submit" className="w-full" disabled={isAuthenticationLoading || save.isPending || !label.trim()}>
